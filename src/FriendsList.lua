...
OnPlayerLogin(function()
  function FriendsListByClassColor_Update()
    local Config = {
      format =
      "[if=level][color=level][=level][/color][/if] [color=class][=accountName|name] [if=characterName]([=characterName])[/if][/color]",
        ...
        return (
          text:gsub("|HBNplayer:(.-)|h(.-)|h",
            function(data, displayText)
              return format("|HBNplayer:%s|h%s|h", data,
                (Friends.GetAlias("BN_WHISPER", displayText)))
            end))
      end
    end
