      ...
      if (cfg.t_showTalentText) and ((talentFormat == 1) or (talentFormat == 2)) and (unitCacheRecord.talents.name) then
        ...
        if (cfg.t_colorTalentTextByClass) then
          local classColor = LibFroznFunctions:GetClassColor(unitCacheRecord.classID, 5,
            cfg.enableCustomClassColors and TT_ExtendedConfig.customClassColors or nil);
    ...
    elseif (unitCacheRecord.averageItemLevel) then
      ...
      if (cfg.t_showGearScore) then
        ...
        if (cfg.t_gearScoreAlgorithm == LFF_GEAR_SCORE_ALGORITHM.TacoTip) then -- TacoTip's GearScore algorithm
          gearScore = (unitCacheRecord.averageItemLevel.TacoTipGearScore > 0) and
          unitCacheRecord.averageItemLevel.TacoTipGearScore or "-";

          if (cfg.t_colorAILAndGSTextByQuality) then
            ailAndGSText:Push(spacer ..
            unitCacheRecord.averageItemLevel.TacoTipGearScoreQualityColor:WrapTextInColorCode(gearScore));
          else
            ailAndGSText:Push(spacer .. gearScore);
          end
        else -- TipTac's GearScore algorithm
          gearScore = (unitCacheRecord.averageItemLevel.TipTacGearScore > 0) and
          unitCacheRecord.averageItemLevel.TipTacGearScore or "-";

          if (cfg.t_colorAILAndGSTextByQuality) then
            ailAndGSText:Push(spacer ..
            unitCacheRecord.averageItemLevel.TipTacGearScoreQualityColor:WrapTextInColorCode(gearScore));
          else
            ailAndGSText:Push(spacer .. gearScore);
          end
        end
